...
### 自动识别信号（按结构特征判别，不靠业务关键词）

读到内容时，命中以下**任一结构特征**即判为流程，自动走本指南，无需用户说“画流程图”：

- 有序步骤：出现“第一步 / 第二步”“1. 2. 3.”“先…再…然后…最后”等时序链。
- 箭头 / 迁移：出现 `A → B`、`->`、状态名之间的转移描述。
- 条件分支：出现“如果…则…否则”“当…时…不然…”“是 / 否”二分。
- 循环 / 重试：出现“重复”“轮询”“直到…为止”“失败则重试”。
- 多角色时序：出现“X 发给 Y，Y 回 Z”的来回（取主线 3–5 步画意境图，精确时序交 mermaid）。

命中后先数主线节点：3–5 直接画；>6 或含数值 / 表格 / 代码 / 多泳道 → 降级 mermaid（可加意境封面）。判别完一句话声明，便于纠偏。
